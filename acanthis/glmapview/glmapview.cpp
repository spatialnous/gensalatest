...
GLMapView::GLMapView() : m_eyePosX(0), m_eyePosY(0) {
    //    connect(this, &QQuickItem::windowChanged, this,
    //    &GLMapView::handleWindowChanged);
...
void GLMapView::handleWindowSync() {
    if (m_dirtyRenderer) {
...
void GLMapView::forceUpdate() { update(); }
...
bool GLMapView::eventFilter(QObject *object, QEvent *e) {
...
void GLMapView::zoomBy(float dzf, int mouseX, int mouseY) {
    ...
    m_eyePosX += (m_zoomFactor - pzf) * screenRatio * GLfloat(mouseX - width() * 0.5f) /
                 GLfloat(width());
    m_eyePosY -=
        (m_zoomFactor - pzf) * GLfloat(mouseY - height() * 0.5f) / GLfloat(height());
    update();
}

void GLMapView::panBy(int dx, int dy) {
...
Point2f GLMapView::getWorldPoint(const QPoint &screenPoint) {
    return Point2f(
        +m_zoomFactor * float(screenPoint.x() - width() * 0.5) / height() - m_eyePosX,
        -m_zoomFactor * float(screenPoint.y() - height() * 0.5) / height() - m_eyePosY);
}

QPoint GLMapView::getScreenPoint(const Point2f &worldPoint) {
    return QPoint((worldPoint.x + m_eyePosX) * height() / m_zoomFactor + width() * 0.5,
                  -(worldPoint.y + m_eyePosY) * height() / m_zoomFactor + height() * 0.5);
}

void GLMapView::resetView() {
