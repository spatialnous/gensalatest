...
#include "glpixelmap.h"
#include "salalib/geometrygenerators.h"
#include "salalib/linkutils.h"

void GLPixelMap::loadGLObjects() {
    QtRegion region = m_pixelMap.getRegion();
    m_rasterTexture.loadRegionData(region.bottom_left.x, region.bottom_left.y,
                                   region.top_right.x, region.top_right.y);
    ...
    if (m_showLinks) {
        ...
        const std::vector<Point2f> &linkFillTriangles =
            GeometryGenerators::generateMultipleDiskTriangles(
                32, m_pixelMap.getSpacing() * 0.25, mergedPixelLocations);
...
void GLPixelMap::loadGLObjectsRequiringGLContext() {
...
void GLPixelMap::paintGL(const QMatrix4x4 &m_mProj, const QMatrix4x4 &m_mView,
...
void GLPixelMap::highlightHoveredPixels(const QtRegion &region) {
    ...
    if (!points.empty()) {
        ...
        for (Point point : points) {
            ...
            lines.push_back(SimpleLine(loc.x - m_pixelMap.getSpacing() * 0.5,
                                       loc.y - m_pixelMap.getSpacing() * 0.5,
                                       loc.x - m_pixelMap.getSpacing() * 0.5,
                                       loc.y + m_pixelMap.getSpacing() * 0.5));
            lines.push_back(SimpleLine(loc.x - m_pixelMap.getSpacing() * 0.5,
                                       loc.y + m_pixelMap.getSpacing() * 0.5,
                                       loc.x + m_pixelMap.getSpacing() * 0.5,
...
void GLPixelMap::highlightHoveredPixels(const std::set<PixelRef> &refs) {
    // n.b., assumes constrain set to true (for if you start the selection off the
    // grid)
    std::vector<Point> points;
    PixelRef hoverPixel = -1;
