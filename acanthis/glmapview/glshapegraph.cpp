...
void GLShapeGraph::loadGLObjects() {
    GLShapeMap::loadGLObjects();
    ...
    std::vector<SimpleLine> linkFillPerimeters =
        GeometryGenerators::generateMultipleCircleLines(
            32, m_shapeGraph.getSpacing() * 0.1, linkPointLocations);
    linkFillPerimeters.insert(linkFillPerimeters.end(), linkLines.begin(),
                              linkLines.end());
    ...
    const std::vector<Point2f> &unlinkFillTriangles =
        GeometryGenerators::generateMultipleDiskTriangles(
            32, m_shapeGraph.getSpacing() * 0.1, unlinkPoints);
