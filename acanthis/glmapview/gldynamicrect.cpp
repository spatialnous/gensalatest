...
void GLDynamicRect::initializeGL(bool m_core) {
    ...
    m_program->addShaderFromSourceCode(
        QOpenGLShader::Vertex, m_core ? vertexShaderSourceCore : vertexShaderSource);
...
void GLDynamicRect::setFillColour(const QColor &colour) {
    m_colour_fill.setX(colour.redF());
    m_colour_fill.setY(colour.greenF());
    m_colour_fill.setZ(colour.blueF());
}

void GLDynamicRect::setStrokeColour(const QColor &colour) {
