            ...
            SampleAnalysis analyse(const IConfig &cfg, FDuration *first, FDuration *last) {
                if (!cfg.benchmarkNoAnalysis()) {
                    ...
                    auto analysis = Catch::Benchmark::Detail::analyse_samples(
                        cfg.benchmarkConfidenceInterval(), cfg.benchmarkResamples(), samples.data(),
...
namespace Catch {
    namespace Benchmark {
        namespace Detail {
            ...
            bootstrap_analysis analyse_samples(double confidence_level, unsigned int n_resamples,
                                               double *first, double *last) {
...
namespace Catch {
    ...
    Approx::Approx(double value)
        : m_epsilon(static_cast<double>(std::numeric_limits<float>::epsilon()) * 100.),
          m_margin(0.0), m_scale(0.0), m_value(value) {}

    Approx Approx::custom() { return Catch::Approx(0); }
    ...
    namespace literals {
        Approx operator""_a(long double val) { return Catch::Approx(val); }
        Approx operator""_a(unsigned long long val) { return Catch::Approx(val); }
...
namespace Catch {
    ...
    namespace {
        ...
        static Optional<bazelShardingOptions> readBazelShardingOptions() {
            ...
            if (!shardIndex) {
                Catch::cerr() << "Warning: could not parse 'TEST_SHARD_INDEX' ('" << bazelShardIndex
                              << "') as unsigned int.\n";
                return {};
            }
            auto shardTotal = parseUInt(bazelShardTotal);
            if (!shardTotal) {
                Catch::cerr() << "Warning: could not parse 'TEST_TOTAL_SHARD' ('" << bazelShardTotal
                              << "') as unsigned int.\n";
                return {};
            }
    ...
    bool operator==(ProcessedReporterSpec const &lhs, ProcessedReporterSpec const &rhs) {
        return lhs.name == rhs.name && lhs.outputFilename == rhs.outputFilename &&
               lhs.colourMode == rhs.colourMode && lhs.customOptions == rhs.customOptions;
    }
    ...
    namespace {
        ...
        class RegistryHub : public IRegistryHub,
          ...
          public: // IMutableRegistryHub
            ...
            void
            registerTranslator(Detail::unique_ptr<IExceptionTranslator> &&translator) override {
        ...
        IEventListenerPtr prepareReporters(Config const *config) {
            if (Catch::getRegistryHub().getReporterRegistry().getListeners().empty() &&
                ...
                return createReporter(spec.name,
                                      ReporterConfig(config, makeStream(spec.outputFilename),
    ...
    int Session::runInternal() {
        ...
        if (m_configData.shardIndex >= m_configData.shardCount) {
            Catch::cerr() << "The shard count (" << m_configData.shardCount
                          << ") must be greater than the shard index (" << m_configData.shardIndex
                          << ")\n"
                          << std::flush;
            return 1;
        }
...
namespace Catch {
    ...
    Detail::unique_ptr<TestCaseInfo> makeTestCaseInfo(StringRef _className,
                                                      NameAndTags const &nameAndTags,
                                                      SourceLineInfo const &_lineInfo) {
        return Detail::make_unique<TestCaseInfo>(_className, nameAndTags, _lineInfo);
    }

    TestCaseInfo::TestCaseInfo(StringRef _className, NameAndTags const &_nameAndTags,
                               SourceLineInfo const &_lineInfo)
        ...
        for (size_t idx = 0; idx < originalTags.size(); ++idx) {
            ...
            if (c == '[') {
                CATCH_ENFORCE(!inTag, "Found '[' inside a tag while registering test case '"
            ...
            if (c == ']') {
                ...
                CATCH_ENFORCE(!tagStr.empty(), "Found an empty tag while registering test case '"
                                                   << _nameAndTags.name << "' at " << _lineInfo);
...
namespace Catch {
    namespace Generators {
        ...
        RandomFloatingGenerator<long double>::RandomFloatingGenerator(long double a, long double b,
                                                                      std::uint32_t seed)
...
namespace Catch {
    ...
    ReporterConfig::ReporterConfig(IConfig const *_fullConfig, Detail::unique_ptr<IStream> _stream,
                                   ColourMode colourMode,
                                   std::map<std::string, std::string> customOptions)
        : m_stream(CATCH_MOVE(_stream)), m_fullConfig(_fullConfig), m_colourMode(colourMode),
    ...
    SectionStats::SectionStats(SectionInfo &&_sectionInfo, Counts const &_assertions,
                               double _durationInSeconds, bool _missingAssertions)
        : sectionInfo(CATCH_MOVE(_sectionInfo)), assertions(_assertions),
          durationInSeconds(_durationInSeconds), missingAssertions(_missingAssertions) {}

    TestCaseStats::TestCaseStats(TestCaseInfo const &_testInfo, Totals const &_totals,
                                 std::string &&_stdOut, std::string &&_stdErr, bool _aborting)
        : testInfo(&_testInfo), totals(_totals), stdOut(CATCH_MOVE(_stdOut)),
          stdErr(CATCH_MOVE(_stdErr)), aborting(_aborting) {}
...
namespace Catch {
    namespace Clara {
        ...
        void Parser::writeToStream(std::ostream &os) const {
            ...
            for (auto &cols : rows) {
                auto row = TextFlow::Column(CATCH_MOVE(cols.left)).width(optWidth).indent(2) +
                           TextFlow::Spacer(4) +
    ...
    Clara::Parser makeCommandLineParser(ConfigData &config) {
        ...
        auto const setWarning = [&](std::string const &warning) {
            if (warning == "NoAssertions") {
                ...
                config.warnings =
                    static_cast<WarnAbout::What>(config.warnings | WarnAbout::UnmatchedTestSpec);
                return ParserResult::ok(ParseResultType::Matched);
            }
        ...
        auto const setReporter = [&](std::string const &userReporterSpec) {
            ...
            if (result == factories.end()) {
                return ParserResult::runtimeError("Unrecognized reporter, '" + reporterSpec.name() +
        ...
        auto cli =
            ...
            Opt(accept_many, setReporter,
                "name[::key=value]*")["-r"]["--reporter"]("reporter to use (defaults to console)") |
...
namespace Catch {
    ...
    namespace {
        static std::string tryTranslators(
            std::vector<Detail::unique_ptr<IExceptionTranslator const>> const &translators) {
...
namespace Catch {
    ...
    JsonObjectWriter::JsonObjectWriter(JsonObjectWriter &&source) noexcept
        : m_os{source.m_os}, m_indent_level{source.m_indent_level},
          m_should_comma{source.m_should_comma}, m_active{source.m_active} {
...
namespace Catch {
    ...
    namespace {
        ...
        kvPair splitKVPair(StringRef kvString) {
            auto splitPos = static_cast<size_t>(std::find(kvString.begin(), kvString.end(), '=') -
                                                kvString.begin());
    ...
    ReporterSpec::ReporterSpec(std::string name, Optional<std::string> outputFileName,
                               Optional<ColourMode> colourMode,
                               std::map<std::string, std::string> customOptions)
        : m_name(CATCH_MOVE(name)), m_outputFileName(CATCH_MOVE(outputFileName)),
          m_colourMode(CATCH_MOVE(colourMode)), m_customOptions(CATCH_MOVE(customOptions)) {}
...
namespace Catch {
    ...
    RunContext::RunContext(IConfig const *_config, IEventListenerPtr &&reporter)
        : m_runInfo(_config->name()), m_config(_config),
          m_reporter(CATCH_MOVE(reporter)), m_lastAssertionInfo{StringRef(), SourceLineInfo("", 0),
                                                                StringRef(),
    ...
    Totals RunContext::runTest(TestCaseHandle const &testCase) {
        ...
        do {
            ...
            auto statsForOneRun = TestCaseStats(testInfo, singleRunTotals, CATCH_MOVE(oneRunCout),
        ...
        m_reporter->testCaseEnded(TestCaseStats(testInfo, deltaTotals, CATCH_MOVE(redirectedCout),
    ...
    bool RunContext::sectionStarted(StringRef sectionName, SourceLineInfo const &sectionLineInfo,
                                    Counts &assertions) {
        ITracker &sectionTracker = SectionTracker::acquire(
            m_trackerContext, TestCaseTracking::NameAndLocationRef(sectionName, sectionLineInfo));
    ...
    IGeneratorTracker *RunContext::acquireGeneratorTracker(StringRef generatorName,
                                                           SourceLineInfo const &lineInfo) {
    ...
    IGeneratorTracker *
    RunContext::createGeneratorTracker(StringRef generatorName, SourceLineInfo lineInfo,
    ...
    void RunContext::sectionEnded(SectionEndInfo &&endInfo) {
        ...
        m_reporter->sectionEnded(SectionStats(CATCH_MOVE(endInfo.sectionInfo), assertions,
    ...
    void RunContext::benchmarkPreparing(StringRef name) { m_reporter->benchmarkPreparing(name); }
    ...
    void RunContext::benchmarkFailed(StringRef error) { m_reporter->benchmarkFailed(error); }
    ...
    void RunContext::handleFatalErrorCondition(StringRef message) {
        ...
        // Since this is a fatal error, we have not had and won't have the opportunity to destruct
        // them properly
    ...
    void RunContext::runCurrentTest(std::string &redirectedCout, std::string &redirectedCerr) {
        ...
        SectionStats testCaseSectionStats(CATCH_MOVE(testCaseSection), assertions, duration,
                                          missingAssertions);
        m_reporter->sectionEnded(testCaseSectionStats);
    }
    ...
    void RunContext::handleMessage(AssertionInfo const &info, ResultWas::OfType resultType,
                                   StringRef message, AssertionReaction &reaction) {
        m_lastAssertionInfo = info;
...
namespace Catch {
    ...
    Section::Section(SectionInfo &&info)
        : m_info(CATCH_MOVE(info)), m_sectionIncluded(getResultCapture().sectionStarted(
...
namespace Catch {
    ...
    void StartupExceptionRegistry::add(std::exception_ptr const &exception) noexcept {
        ...
        CATCH_CATCH_ALL {
            // If we run out of memory during start-up there's really not a lot more we can do about
            // it
            std::terminate();
        }
    }
    ...
    namespace {
        static void enforceNoDuplicateTestCases(std::vector<TestCaseHandle> const &tests) {
            ...
            for (auto const &test : tests) {
                ...
                CATCH_ENFORCE(prev.second, "error: test case \""
...
namespace Catch {
    namespace TestCaseTracking {
        ...
        SectionTracker::SectionTracker(NameAndLocation &&nameAndLocation, TrackerContext &ctx,
                                       ITracker *parent)
            : TrackerBase(CATCH_MOVE(nameAndLocation), ctx, parent),
    ...
    AutoReg::AutoReg(Detail::unique_ptr<ITestInvoker> invoker, SourceLineInfo const &lineInfo,
        ...
        CATCH_TRY {
            getMutableRegistryHub().registerTest(
                makeTestCaseInfo(extractClassName(classOrMethod), nameAndTags, lineInfo),
                CATCH_MOVE(invoker));
        }
        CATCH_CATCH_ALL {
            // Do not throw when constructing global objects, instead register the exception to be
...
namespace Catch {
    namespace TextFlow {
        ...
        std::string AnsiSkippingString::substring(const_iterator begin, const_iterator end) const {
            ...
            // 0xff back to 'm'
            auto str =
                std::string(begin == this->begin() ? m_string.begin() : begin.m_it, end.m_it);
        ...
        void AnsiSkippingString::const_iterator::tryParseAnsiEscapes() {
            ...
            // it
            while (m_it != m_string->end() && *m_it == '\033' && m_it + 1 != m_string->end() &&
                   *(m_it + 1) == '[') {
                auto cursor = m_it + 2;
                while (cursor != m_string->end() && (isdigit(*cursor) || *cursor == ';')) {
                    ++cursor;
        ...
        static bool isBoundary(AnsiSkippingString const &line,
            ...
            return it == line.end() || (isWhitespace(*it) && !isWhitespace(*it.oneBefore())) ||
        ...
        size_t Column::const_iterator::indentSize() const {
            auto initial = m_lineStart == m_column.m_string.begin() ? m_column.m_initialIndent
                                                                    : std::string::npos;
        ...
        Column::const_iterator::const_iterator(Column const &column)
            : m_column(column), m_lineStart(column.m_string.begin()),
              m_lineEnd(column.m_string.begin()), m_parsedTo(column.m_string.begin()) {
...
namespace Catch {
    ...
    namespace {
        ...
        bool shouldNewline(XmlFormatting fmt) {
            return !!(
                static_cast<std::underlying_type_t<XmlFormatting>>(fmt & XmlFormatting::Newline));
        }

        bool shouldIndent(XmlFormatting fmt) {
            return !!(
                static_cast<std::underlying_type_t<XmlFormatting>>(fmt & XmlFormatting::Indent));
        }
    ...
    XmlEncode::XmlEncode(StringRef str, ForWhat forWhat) : m_str(str), m_forWhat(forWhat) {}
    ...
    XmlWriter::ScopedElement &XmlWriter::ScopedElement::writeAttribute(StringRef name,
                                                                       StringRef attribute) {
...
namespace Catch {
    ...
    // There is another overload, in catch_assertionhandler.h/.cpp, that only takes a string and
    // infers the Equals matcher (so the header does not mention matchers)
...
namespace Catch {
    ReporterBase::ReporterBase(ReporterConfig &&config)
        : IEventListener(config.fullConfig()), m_wrapped_stream(CATCH_MOVE(config).takeStream()),
...
namespace Catch {
          ...
          public:
            ...
            ConsoleAssertionPrinter(std::ostream &_stream, AssertionStats const &_stats,
                ...
                : stream(_stream), stats(_stats), result(_stats.assertionResult),
                  colour(Colour::None), messages(_stats.infoMessages), colourImpl(colourImpl_),
    ...
    ConsoleReporter::ConsoleReporter(ReporterConfig &&config)
        : StreamingReporterBase(CATCH_MOVE(config)),
          m_tablePrinter(
              Detail::make_unique<TablePrinter>(m_stream, [&config]() -> std::vector<ColumnInfo> {
    ...
    void ConsoleReporter::lazyPrintRunInfo() {
        m_stream << '\n'
                 << lineOfChars('~') << '\n'
                 << m_colour->guardColour(Colour::SecondaryText) << currentTestRunInfo.name
...
namespace Catch {
    namespace {
        struct BySectionInfo {
            ...
            bool
            operator()(Detail::unique_ptr<CumulativeReporterBase::SectionNode> const &node) const {
    ...
    bool CumulativeReporterBase::SectionNode::hasAnyAssertions() const {
        return std::any_of(
            assertionsAndBenchmarks.begin(), assertionsAndBenchmarks.end(),
...
namespace Catch {
    ...
    std::string getFormattedDuration(double duration) {
        ...
        size_t printedLength =
            static_cast<size_t>(std::snprintf(buffer, maxDoubleSize, "%.3f", duration));
#endif
    ...
    void defaultListReporters(std::ostream &out,
        ...
        const auto maxNameLen =
            std::max_element(descriptions.begin(), descriptions.end(),
...
namespace Catch {
    ...
    void JsonReporter::testCaseEnded(TestCaseStats const &tcStats) {
...

        {
            ...
            // We do not write the test case totals, because there will always be just one test case
            // here.
...
namespace Catch {
    ...
    void SonarQubeReporter::writeTestFile(StringRef filename,
    ...
    void SonarQubeReporter::writeSection(std::string const &rootName,
                                         SectionNode const &sectionNode, bool okToFail) {
        std::string name = trim(sectionNode.stats.sectionInfo.name);
        if (!rootName.empty())
            name = rootName + '/' + name;
        ...
        if (sectionNode.hasAnyAssertions() || !sectionNode.stdOut.empty() ||
            ...
            XmlWriter::ScopedElement e = xml.scopedElement("testCase");
            xml.writeAttribute("name"_sr, name);
            xml.writeAttribute("duration"_sr,
        ...
        class TapAssertionPrinter {
          public:
            ...
            TapAssertionPrinter(std::ostream &_stream, AssertionStats const &_stats,
                ...
                : stream(_stream), result(_stats.assertionResult), messages(_stats.infoMessages),
                  itMessage(_stats.infoMessages.begin()), printInfoMessages(true),
                  counter(_counter), colourImpl(colour_) {}
...
namespace Catch {
    ...
    void TeamCityReporter::assertionEnded(AssertionStats const &assertionStats) {
        ...
        if (!result.isOk() || result.getResultType() == ResultWas::ExplicitSkip) {
            ...
            m_stream << " name='" << escape(currentTestCaseInfo->name) << '\'' << " message='"
                     << escape(msg.str()) << '\'' << "]\n";
        }
        m_stream.flush();
    }
...
namespace Catch {
    ...
    void XmlReporter::sectionStarting(SectionInfo const &sectionInfo) {
        ...
        if (m_sectionDepth++ > 0) {
            m_xml.startElement("Section").writeAttribute("name"_sr,
    ...
    void XmlReporter::testCaseEnded(TestCaseStats const &testCaseStats) {
        ...
        if (!testCaseStats.stdOut.empty())
            m_xml.scopedElement("StdOut").writeText(trim(StringRef(testCaseStats.stdOut)),
                                                    XmlFormatting::Newline);
        if (!testCaseStats.stdErr.empty())
            m_xml.scopedElement("StdErr").writeText(trim(StringRef(testCaseStats.stdErr)),
                                                    XmlFormatting::Newline);
