...
TEST_CASE("VisPrepParserFail", "Error cases") {
    ...
    SECTION("Non-numeric input to -pg") {
        ...
        REQUIRE_THROWS_WITH(parser.parse(ah.argc(), ah.argv()),
                            Catch::Contains("-pg must be a number >0, got foo"));
    }
    ...
    SECTION("Neither points nor point file provided") {
        ...
        REQUIRE_THROWS_WITH(parser.parse(ah.argc(), ah.argv()),
                            Catch::Contains("Either -pp or -pf must be given"));
    }
    ...
    SECTION("Points and pointfile provided") {
        ...
        REQUIRE_THROWS_WITH(parser.parse(ah.argc(), ah.argv()),
                            Catch::Contains("-pf cannot be used together with -pp"));
    }
    ...
    SECTION("Pointfile and points provided") {
        ...
        REQUIRE_THROWS_WITH(parser.parse(ah.argc(), ah.argv()),
                            Catch::Contains("-pp cannot be used together with -pf"));
    }
    ...
    SECTION("Nonsensical visibility restriction") {
        ...
        REQUIRE_THROWS_WITH(parser.parse(ah.argc(), ah.argv()),
                            Catch::Contains("Restricted visibility of 'foo' makes no sense, use a "
    ...
    SECTION("Nonsensical visibility restriction") {
        ...
        REQUIRE_THROWS_WITH(parser.parse(ah.argc(), ah.argv()),
                            Catch::Contains("Restricted visibility of '0.0' makes no sense, use a "
                                            "positive number or -1 for unrestricted"));
    }
    ...
    SECTION("Make and unmake") {
        ...
        REQUIRE_THROWS_WITH(parser.parse(ah.argc(), ah.argv()),
                            Catch::Contains("-pu cannot be used together with -pm"));
    }
    ...
    SECTION("Grid and unmake") {
        ...
        REQUIRE_THROWS_WITH(
            parser.parse(ah.argc(), ah.argv()),
            Catch::Contains("-pu can not be used with any other option apart from -pl"));
    }
}
