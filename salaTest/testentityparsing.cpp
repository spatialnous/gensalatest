...
TEST_CASE("Failing line parser", "") {
    const float EPSILON = 0.001f;
    {
        ...
        REQUIRE_THROWS_WITH(
            EntityParsing::parseLines(stream, ','),
            Catch::Contains("Badly formatted header (should contain x1, y1, x2 and y2)"));
    }
...

    {
        ...
        REQUIRE_THROWS_WITH(EntityParsing::parseLines(stream, ','),
                            Catch::Contains("Error parsing line"));
    }
}
...
TEST_CASE("Failing point parser", "") {
    const float EPSILON = 0.001f;
    {
        ...
        REQUIRE_THROWS_WITH(EntityParsing::parsePoints(stream, ','),
                            Catch::Contains("Badly formatted header (should contain x and y)"));
    }
...
TEST_CASE("Test point parsing") {
    REQUIRE_THROWS_WITH(
        EntityParsing::parsePoint("foo", '|'),
        Catch::Contains("Badly formatted point data, should be <number>|<number>, was foo"));
...
TEST_CASE("Failing Isovist parser") {
    {
        ...
        REQUIRE_THROWS_WITH(EntityParsing::parseIsovists(stream, ','),
                            Catch::Contains("Badly formatted header (should contain x and y, might "
...

    {
        ...
        REQUIRE_THROWS_WITH(EntityParsing::parseIsovists(stream, ','),
                            Catch::Contains("Error parsing line: 1.0,1.0,270"));
    }
}
...
TEST_CASE("Parsing single isovist") {
    ...
    SECTION("Failed bad string") {
        REQUIRE_THROWS_WITH(EntityParsing::parseIsovist("1,1,27"),
                            Catch::Contains("Failed to parse '1,1,27' to an isovist definition"));
    }
}
